...
void ProjectManager::createFile()
{
    ...
    {
        throwObjectAlreadyExistsError("file");
    }
}
void ProjectManager::createFunction()
{
    ...
    if (!promptErrorIfMain(fileName)) {return;}
    ...
    {
        throwObjectAlreadyExistsError("function");
    }
...
void ProjectManager::createClass()
{
    ...
    if (!promptErrorIfUtil(fileName)) {return;}
    ...
    if (Checker.checkFileAlreadyExists(projectName + "/.PROJECT", fileName))
    {
        ...
        {
            throwObjectAlreadyExistsError("class");
        }
    }
    else
    {
        throwObjectDoesntExistError("file");
    }
...
void ProjectManager::createVariable()
{
    ...
    if (Checker.checkFileAlreadyExists(projectName + "/.PROJECT", fileName))
    {
        ...
        {
            throwObjectAlreadyExistsError("variable");
        }
    }
    else
    {
...
void ProjectManager::createStruct()
{
    ...
    if (Checker.checkFileAlreadyExists(projectName + "/.PROJECT", fileName))
    {
        ...
        {
            throwObjectAlreadyExistsError("struct");
        }
    }
    else
    {
...
void ProjectManager::deleteFile()
{
    ...
    if (Checker.checkFileAlreadyExists(projectName + "/.PROJECT", fileName))
    {
        ...
        if (UserManager.UsersEnabled())
        {
            logEvent(fileName + " file deleted by " + userName + ".", projectName);
        }
        else
        {
            logEvent(fileName + " file deleted.", projectName);
        }
    }       
    else
    {
...
void ProjectManager::deleteFunction()
{
    ...
    if (Checker.checkFunctionAlreadyExists(projectName + "/.PROJECT", functionName))
    {
        // Confirm User wants to delete the function
        if (!promptUserForDeletionConfirmation(functionName)) {return;}

        // Retrieve all header files from metadata
        vector<string> headerFiles = getHeaderFiles(projectName + "/.PROJECT", projectName);
        string headerFile, sourceFile;

        // Search for function definition in header files
        for (const string& file : headerFiles)
        {
            vector<string> structs, classes, variables, functions;
            parseHeaderFile(file, structs, classes, variables, functions);

            if (find(functions.begin(), functions.end(), functionName) != functions.end())
            {
                headerFile = file;
                sourceFile = headerFile;
                sourceFile.replace(sourceFile.find(".h"), 2, ".cpp"); // Convert to corresponding .cpp file
                break;
            }
        }

        // If function was not found, throw an error
        if (headerFile.empty() || sourceFile.empty())
        {
            throwObjectDoesntExistError("function");
            return;
        }

        // Remove function from the header file
        removeFunctionFromFile(headerFile, functionName);

        // Remove function from the source file
        removeFunctionFromFile(sourceFile, functionName);

        // Update metadata to remove function reference
        updateFunctionMetadata(projectName + "/.PROJECT", functionName);

        // Log the deletion
        if (UserManager.UsersEnabled())
        {
            logEvent(functionName + " function deleted by " + userName + ".", projectName);
        }
        else
        {
            logEvent(functionName + " function deleted.", projectName);
        }
    }
    else
    {
...
void ProjectManager::deleteClass()
{
    ...
    if (Checker.checkClassAlreadyExists(projectName + "/.PROJECT", className))
    {
        if (!promptUserForDeletionConfirmation(className)) {return;}

        vector<string> headerFiles = getHeaderFiles(projectName + "/.PROJECT", projectName);
        string headerFile, sourceFile;

        // Search for class definition in header files
        for (const string& file : headerFiles)
        {
            vector<string> structs, classes, variables, functions;
            parseHeaderFile(file, structs, classes, variables, functions);
        ...
        // If class was not found, throw an error
        if (headerFile.empty() || sourceFile.empty())
        {
            throwObjectDoesntExistError("class");
            return;
        }

        // Remove class from the header file
        removeClassFromFile(headerFile, className);

        // Update metadata to remove class reference
        updateClassMetadata(projectName + "/.PROJECT", className);

        // Log the deletion
        if (UserManager.UsersEnabled())
        {
            logEvent(className + " class deleted by " + userName + ".", projectName);
        }
        else
        {
            logEvent(className + " class deleted.", projectName);
        }
    }
...
void ProjectManager::deleteStruct()
{
    ...
    if (Checker.checkStructAlreadyExists(projectName + "/.PROJECT", structName))
    {
        if (!promptUserForDeletionConfirmation(structName)) {return;}

        vector<string> headerFiles = getHeaderFiles(projectName + "/.PROJECT", projectName);
        string headerFile, sourceFile;

        // Search for struct definition in header files
        for (const string& file : headerFiles)
        {
            vector<string> structs, classes, variables, functions;
            parseHeaderFile(file, structs, classes, variables, functions);

            if (find(structs.begin(), structs.end(), structName) != structs.end())
            {
                headerFile = file;
                sourceFile = headerFile;
                sourceFile.replace(sourceFile.find(".h"), 2, ".cpp"); // Convert to corresponding .cpp file
        ...
        // If variable was not found, throw an error
        if (headerFile.empty() || sourceFile.empty())
        {
            throwObjectDoesntExistError("struct");
            return;
        }

        // Remove struct from the header file
        removeStructFromFile(headerFile, structName);

        // Update metadata to remove struct reference
        updateStructMetadata(projectName + "/.PROJECT", structName);

        // Log the deletion
        if (UserManager.UsersEnabled())
        {
            logEvent(structName + " struct deleted by " + userName + ".", projectName);
        }
        else
        {
            logEvent(structName + " struct deleted.", projectName);
        }
    }
}
void ProjectManager::deleteVariable()
{
    ...
    if (Checker.checkVariableAlreadyExists(projectName + "/.PROJECT", variableName))
    {
        if (!promptUserForDeletionConfirmation(variableName)) {return;}
        ...
        for (const string& file : headerFiles)
        {
            ...
            if (find(variables.begin(), variables.end(), variableName) != variables.end())
            {
                headerFile = file;
                sourceFile = headerFile;
                sourceFile.replace(sourceFile.find(".h"), 2, ".cpp"); // Convert to corresponding .cpp file
        ...
        if (headerFile.empty() || sourceFile.empty())
        {
            throwObjectDoesntExistError("variable");
            return;
        }

        // Remove variable from the header file
        removeVariableFromFile(headerFile, variableName);

        // Remove variable from the source file
        removeVariableFromFile(sourceFile, variableName);

        // Update metadata to remove variable reference
        updateVariableMetadata(projectName + "/.PROJECT", variableName);

        // Log the deletion
        if (UserManager.UsersEnabled())
        {
            logEvent(variableName + " variable deleted by " + userName + ".", projectName);
        }
        else
        {
            logEvent(variableName + " variable deleted.", projectName);
        }
    }
}
...
void ProjectManager::logComment()
{
    UserManager UserManager;
    string userComment;
    cout << "Enter a comment to add to the log file: ";
    getline(cin, userComment);

    if (UserManager.UsersEnabled())
    {
        logEvent(userName + ": " + userComment + ".", projectName);
    }
    else
    {
        logEvent("Comment: " + userComment + ".", projectName);
    }
}
